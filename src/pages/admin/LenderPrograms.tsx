        ...
        <div className="bg-white rounded-md border border-slate-200 overflow-hidden">
          <ScrollArea className="h-[calc(100vh-240px)]">
            <div style={{ minWidth: COLUMNS.reduce((sum, c) => sum + c.width, 50) }}>
              {/* Header */}
              <div className="flex bg-slate-100 border-b-2 border-slate-300 sticky top-0 z-10">
                {COLUMNS.map((col) => (
                  <div
                    key={col.key}
                    className="px-3 py-3 text-sm font-semibold text-slate-700 border-r border-slate-200 last:border-r-0 flex-shrink-0"
              ...
              {filteredRows.map((row) => (
                <div
                  key={row.id}
                  className={`flex border-b border-slate-200 hover:bg-slate-50 group min-h-[48px] ${
                ...
                >
                  {COLUMNS.map((col) => {
                    ...
                    return (
                      <div
                        key={col.key}
                        className={`px-2 py-2 border-r border-slate-100 last:border-r-0 flex-shrink-0 flex ${
                          isLookingFor ? 'items-start' : 'items-center'
                        } ${
                          col.editable ? 'cursor-text' : ''
                        } ${col.key === 'rowNum' ? 'bg-slate-50 text-slate-500 text-sm font-medium justify-center' : ''}`}
                        style={{ width: col.width }}
                        ...
                        {isEditing ? (
                          isLookingFor ? (
                            <textarea
                              ...
                              className="w-full h-20 text-sm px-2 py-1 border border-blue-400 rounded focus:outline-none focus:ring-1 focus:ring-blue-400 resize-none"
                            />
                          ) : (
                            <Input
                              autoFocus
                              ...
                              className="h-8 text-sm px-2 border-blue-400 focus-visible:ring-1 focus-visible:ring-blue-400"
                            />
                          )
                        ) : (
