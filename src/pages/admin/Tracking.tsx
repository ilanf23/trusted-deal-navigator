...
const Tracking = () => {
  ...
  const { data: trackingRecords = [], isLoading } = useQuery({
    queryKey: ['admin-partner-tracking'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('partner_tracking')
        .select('*, referral:partner_referrals(id, name, company_name, status, loan_type, loan_amount, email, phone, partner_id)')
        .order('created_at', { ascending: false });
      if (error) throw error;

      // Fetch partner profiles for display
      const partnerIds = [...new Set((data || []).map((t: any) => t.partner_id))];
      ...
      return (data || []).map((t: any) => ({
        ...t,
        partner_profile: profilesMap[t.partner_id] || null,
      })) as TrackingRecord[];
    },
    enabled: !!user,
  });

  // Fetch all unlinked referrals for the link dialog
  const { data: unlinkedReferrals = [] } = useQuery({
    queryKey: ['admin-unlinked-referrals', trackingRecords],
    queryFn: async () => {
      const linkedIds = trackingRecords.map((t) => t.referral_id);
      const { data, error } = await supabase
        .from('partner_referrals')
        .select('id, name, company_name, loan_type, loan_amount, status, partner_id');
      if (error) throw error;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'partner_tracking' }, () => {
        queryClient.invalidateQueries({ queryKey: ['admin-partner-tracking'] });
      })
      .subscribe();
  ...
  const linkReferral = useMutation({
    mutationFn: async (referral: { id: string; partner_id: string }) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-partner-tracking'] });
      queryClient.invalidateQueries({ queryKey: ['admin-unlinked-referrals'] });
  ...
  const updateTracking = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-partner-tracking'] }),
  ...
  const getPartnerName = (t: TrackingRecord) => {
    if (t.partner_profile?.first_name || t.partner_profile?.last_name) {
      return `${t.partner_profile.first_name || ''} ${t.partner_profile.last_name || ''}`.trim();
    }
          ...
          <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
            ...
            <DialogContent>
              ...
              {unlinkedReferrals.length === 0 ? (
                <p className="text-sm text-muted-foreground py-4">All referrals are already linked.</p>
              ) : (
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {unlinkedReferrals.map((r) => (
                    <Card key={r.id} className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => linkReferral.mutate({ id: r.id, partner_id: r.partner_id })}>
                ...
                <TableBody>
                  {filtered.map((t) => (
                    <TableRow key={t.id}>
                      <TableCell>
                        <div>
                          <p className="font-medium text-sm">{getPartnerName(t)}</p>
                          {t.partner_profile?.company && <p className="text-xs text-muted-foreground">{t.partner_profile.company}</p>}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
