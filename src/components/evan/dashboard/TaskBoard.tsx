...
import { Plus, ChevronDown, Star, Trash2, MessageSquare, Search, Filter, SlidersHorizontal, EyeOff, ArrowUpDown } from 'lucide-react';
...
const statusConfig: Record<string, { label: string; bg: string; text: string; hoverBg: string }> = {
  working: { label: 'Working on it', bg: 'bg-[#fdab3d]', text: 'text-white', hoverBg: 'hover:bg-[#e69a2e]' },
  stuck: { label: 'Stuck', bg: 'bg-[#e2445c]', text: 'text-white', hoverBg: 'hover:bg-[#ce3048]' },
  done: { label: 'Done', bg: 'bg-[#00c875]', text: 'text-white', hoverBg: 'hover:bg-[#00b066]' },
  todo: { label: "Haven't started yet", bg: 'bg-[#579bfc]', text: 'text-white', hoverBg: 'hover:bg-[#4589e6]' },
  reschedule: { label: 'Reschedule', bg: 'bg-[#a25ddc]', text: 'text-white', hoverBg: 'hover:bg-[#9050c9]' },
  blank: { label: '', bg: 'bg-[#c4c4c4]', text: 'text-white', hoverBg: 'hover:bg-[#b0b0b0]' },
};

const getStatusBorderColor = (status: string | null): string => {
  switch (status) {
    case 'working': return 'border-l-[#fdab3d]';
    case 'stuck': return 'border-l-[#e2445c]';
    case 'done': return 'border-l-[#00c875]';
    case 'todo': return 'border-l-[#579bfc]';
    case 'reschedule': return 'border-l-[#a25ddc]';
                  ...
                  <CollapsibleContent>
                    ...
                    {dayTasks.length === 0 ? (
                      ...
                      dayTasks.map((task) => (
                        <div
                          key={task.id}
                          className={`grid grid-cols-[40px_40px_1fr_32px_100px_140px_140px_80px_100px_40px] gap-0 border-b border-border/50 items-center hover:bg-muted/20 transition-colors group border-l-[3px] ${getStatusBorderColor(task.status)}`}
                        >
                          {/* Checkbox */}
                          ...
                          <div className="p-2">
                            <Input
                              ...
                              className="h-7 border-0 bg-transparent p-0 focus-visible:ring-0 text-sm font-normal"
                            />
                          </div>
