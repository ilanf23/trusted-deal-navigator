...
import { ChevronDown, Plus, Trash2, Eye, EyeOff, Lock, ArrowLeft, ArrowRight, Type, Calendar, CheckSquare, Tag, Code, UserCircle, Contact, Sparkles, Clock, Mail, Phone, MessageSquare, ListChecks, TrendingUp, User, RefreshCw, Activity, FileText, Users, Hash, Eye as EyeIcon, Video, Mic, Zap } from 'lucide-react';
...
const magicColumnCategories: {
  category: string;
  icon: React.ElementType;
  columns: MagicColumnConfig[];
}[] = [{
  ...
  columns: [{
    ...
    icon: User,
    label: 'Created By',
    description: 'Who created this lead',
    category: 'Creation Data'
  }, {
    type: 'created_date',
    icon: Calendar,
    label: 'Created Date',
    description: 'When this lead was created',
    category: 'Creation Data'
  }, {
    type: 'days_in_pipeline',
    icon: Clock,
    label: 'Days in Pipeline',
    description: 'Total time in the pipeline',
    category: 'Creation Data'
  }, {
    type: 'last_updated',
    icon: RefreshCw,
    label: 'Last Updated',
    description: 'When this lead was last modified',
    category: 'Creation Data'
  }, {
    type: 'last_updated_by',
    icon: User,
    label: 'Last Updated By',
    description: 'Who last modified this lead',
    category: 'Creation Data'
  }]
}, {
  category: 'Freshness',
  ...
  columns: [{
    ...
    icon: Activity,
    label: 'Freshness',
    description: 'How recently updated vs other leads',
    category: 'Freshness'
  }]
}, {
  category: 'Summary',
  ...
  columns: [{
    ...
    icon: Mail,
    label: 'Email Count',
    description: 'Total emails exchanged',
    category: 'Summary'
  }, {
    type: 'file_count',
    icon: FileText,
    label: 'File Count',
    description: 'Files attached to this lead',
    category: 'Summary'
  }, {
    type: 'comment_count',
    icon: MessageSquare,
    label: 'Comment Count',
    description: 'Internal comments on this lead',
    category: 'Summary'
  }, {
    type: 'task_count',
    icon: ListChecks,
    label: 'Task Count',
    description: 'Total tasks for this lead',
    category: 'Summary'
  }, {
    type: 'call_count',
    icon: Phone,
  ...
  icon: Mail,
  columns: [{
    type: 'last_email_date',
    icon: Mail,
    label: 'Last Email Date',
    description: 'Date of most recent email',
    category: 'Email Data'
  }, {
    type: 'last_email_sent',
    icon: Mail,
    label: 'Last Email Sent',
    description: 'When you last emailed them',
    category: 'Email Data'
  }, {
    type: 'last_email_received',
    icon: Mail,
    label: 'Last Email Received',
    description: 'When they last emailed you',
    category: 'Email Data'
  }, {
    type: 'email_thread_count',
    icon: MessageSquare,
    label: 'Email Threads',
    description: 'Number of email conversations',
    category: 'Email Data'
  }, {
    type: 'unique_email_addresses',
    icon: Users,
    label: 'Unique Emails',
    description: 'Distinct email addresses involved',
    category: 'Email Data'
  }]
}, {
  ...
  icon: EyeIcon,
  columns: [{
    type: 'total_email_views',
    icon: EyeIcon,
    label: 'Total Email Views',
    description: 'How many times emails were opened',
    category: 'Email Tracking'
  }, {
    type: 'last_email_view_date',
    icon: EyeIcon,
    label: 'Last View Date',
    description: 'When email was last viewed',
    category: 'Email Tracking'
  }]
}, {
  category: 'Stage Data',
  ...
  columns: [{
    ...
    icon: Clock,
    label: 'Days in Stage',
    description: 'Time in current stage',
    category: 'Stage Data'
  }, {
    type: 'stage_entered_date',
    icon: Calendar,
    label: 'Stage Entered',
    description: 'When entered current stage',
    category: 'Stage Data'
  }, {
    type: 'stage_changes_count',
    icon: TrendingUp,
    label: 'Stage Changes',
    description: 'Number of stage transitions',
    category: 'Stage Data'
  }, {
    type: 'time_to_current_stage',
    icon: Clock,
    label: 'Time to Stage',
    description: 'Days from creation to current stage',
    category: 'Stage Data'
  }]
}, {
  category: 'Task Data',
  ...
  columns: [{
    ...
    icon: Calendar,
    label: 'Next Task Due',
    description: 'Date of next due task',
    category: 'Task Data'
  }, {
    type: 'task_assignees',
    icon: Users,
    label: 'Task Assignees',
    description: 'Who has tasks assigned',
    category: 'Task Data'
  }]
}, {
  ...
  columns: [{
    ...
    description: 'Email, Call, or Meeting',
    category: 'Contact Data'
  }, {
    ...
    description: 'All communications combined',
    category: 'Contact Data'
  }]
}, {
  ...
  columns: [{
    ...
    description: 'When last call occurred',
    category: 'Call/Meeting Data'
  }, {
    type: 'meeting_notes_count',
    icon: FileText,
    label: 'Meeting Notes',
    description: 'Number of meeting notes',
    category: 'Call/Meeting Data'
  }]
}, {
  category: 'Other',
  ...
  columns: [{
    ...
    description: 'Team members following this lead',
    category: 'Other'
  }, {
    type: 'lead_id',
    icon: Hash,
...
const PipelineColumnHeader = ({
  ...
  const getColumnIcon = () => {
    ...
    if (column.type === 'custom' && column.columnType) {
      ...
      if (customType) {
        ...
        return <Icon className="h-3 w-3" style={{
  ...
  return <div className="flex items-center gap-1.5 group relative w-full">
      {getColumnIcon()}
      <span className={cn("text-sm whitespace-nowrap", column.type === 'magic' && 'text-purple-600', column.isFrozen && 'font-bold')}>
        {column.name}
      </span>
      {column.isFrozen}
      <div className="flex-1" />
      {showDropdown && <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DropdownMenuContent align="start" className="w-56">
            ...
            <DropdownMenuSub>
              ...
              <DropdownMenuSubContent className="w-52">
                ...
                {customColumnTypes.map(colType => <DropdownMenuItem key={colType.type} onClick={() => onInsertColumn?.('left', colType.type)} className="cursor-pointer">
                    <colType.icon className="h-4 w-4 mr-2" style={{
                color: colType.color
              }} />
                    {colType.label}
                  </DropdownMenuItem>)}
                ...
                <ScrollArea className="h-[200px]">
                  {magicColumnCategories.map(cat => <div key={cat.category}>
                      <div className="px-2 py-1 text-[10px] font-medium text-purple-400 uppercase tracking-wider flex items-center gap-1">
                        <cat.icon className="h-3 w-3" />
                        {cat.category}
                      </div>
                      {cat.columns.map(magic => <DropdownMenuItem key={magic.type} onClick={() => onInsertColumn?.('left', magic.type, true)} className="cursor-pointer pl-4">
            ...
            <DropdownMenuSub>
              ...
              <DropdownMenuSubContent className="w-52">
                <div className="px-2 py-1.5 text-xs font-semibold text-slate-500 uppercase">Custom Columns</div>
                {customColumnTypes.map(colType => <DropdownMenuItem key={colType.type} onClick={() => onInsertColumn?.('right', colType.type)} className="cursor-pointer">
                    <colType.icon className="h-4 w-4 mr-2" style={{
                color: colType.color
              }} />
                    {colType.label}
                ...
                <ScrollArea className="h-[200px]">
                  {magicColumnCategories.map(cat => <div key={cat.category}>
                      ...
                      {cat.columns.map(magic => <DropdownMenuItem key={magic.type} onClick={() => onInsertColumn?.('right', magic.type, true)} className="cursor-pointer pl-4">
                          <magic.icon className="h-4 w-4 mr-2 text-purple-500" />
                          {magic.label}
                        </DropdownMenuItem>)}
                    </div>)}
            ...
            {column.canDelete && <>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={onDeleteColumn} className="cursor-pointer text-red-600 focus:text-red-600 focus:bg-red-50">
