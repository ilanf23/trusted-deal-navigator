...
const LeadDetailDialog = ({ lead, open, onOpenChange, onLeadUpdated }: LeadDetailDialogProps) => {
  ...
  // narrowed shape that doesn't carry related_id.
  const { data: leadEntityId = null } = useQuery({
    queryKey: ['lead-related-id', lead?.id],
    queryFn: async () => {
      if (!lead) return null;
      const { data } = await supabase.from('deals').select('related_id').eq('id', lead.id).maybeSingle();
      return data?.related_id ?? null;
    },
    enabled: !!lead && open,
  });
  ...
  const { data: addresses = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!leadEntityId) return [];
      const { data } = await supabase.from('related_addresses').select('*').eq('related_id', leadEntityId);
  ...
  const { data: activities = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!lead) return [];
      const { data } = await supabase.from('activities').select('*').eq('related_id', lead.id).order('created_at', { ascending: false });
  ...
  const updateLeadAssignment = useMutation({
    mutationFn: async (assignedTo: string) => {
      if (!lead) return;
      const { error } = await supabase.from('deals').update({ assigned_to: assignedTo }).eq('id', lead.id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['evans-pipeline-leads'] });
  ...
  const saveNotes = useMutation({
    mutationFn: async () => {
      if (!lead) return;
      const { error } = await supabase.from('deals').update({ notes: notesContent }).eq('id', lead.id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['evans-pipeline-leads'] });
