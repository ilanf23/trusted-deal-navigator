  ...
  const { data: calls = [], isLoading } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const pending = rows.some(
        (r) => r.transcription_status === 'processing' || r.transcription_status === 'pending',
      );
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="text-xs">
                  ...
                  <Clock className="inline h-3 w-3 -mt-0.5" /> {formatDuration(activeCall.duration_seconds)} · {activeCallMember}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                {activeCall.recording_url ? (
                  ...
                  <p className="text-xs text-muted-foreground italic">No recording available for this call.</p>
                )}
                        ...
                        return (
                          <div className="rounded-md border border-rose-200 bg-rose-50/50 p-2.5">
                            <p className="text-xs text-rose-700 flex items-start gap-1.5">
                              ...
                              <span>Transcription failed.{state.error ? ` ${state.error}` : ''}</span>
                            </p>
                          </div>
                        );
