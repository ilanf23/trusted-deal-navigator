-- Harden merge_people: keep the existing authenticated RPC surface for the
-- frontend, but make the SECURITY DEFINER function enforce caller authz before
-- it performs destructive writes.
...
BEGIN
  IF NOT (
    ...
    RAISE EXCEPTION 'merge_people: admin privileges are required'
...
REVOKE ALL ON FUNCTION public.merge_people(uuid, uuid, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.merge_people(uuid, uuid, jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.merge_people(uuid, uuid, jsonb) FROM authenticated;
