-- ============================================================
-- Migration: drop person_connections + person_other_contacts
-- ============================================================
-- These two polymorphic tables were never wired into the app.
-- The only references anywhere were two UPDATE statements inside
-- move_deal_between_pipelines() (step 2, re-pointing entity_type),
-- and both tables were always empty. Per the "remove it if it
-- doesn't work yet" cleanup, we drop them here.
--
-- Because the RPC references the tables, we first recreate the
-- function without those two lines, then drop the tables.
-- ============================================================
...
-- Now that nothing references them, drop the unused tables.
drop table if exists public.person_connections;
drop table if exists public.person_other_contacts;
