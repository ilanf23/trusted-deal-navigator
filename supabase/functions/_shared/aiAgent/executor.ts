...
export async function undoChange(
  ...
  if (operation === "update" && old_values) {
    ...
    // Related-backed tables (people/companies/deals/lender_programs) drop their
    // related row when deleted, so a captured related_id is stale — strip it
    // and let the insert trigger mint a fresh one.
    const { related_id: _staleEntityId, ...restorable } = old_values as Record<string, unknown>;
