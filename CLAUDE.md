...
Key tables: `users`, `deals`, `tasks`, `active_calls`, `call_events`, `communications`, `ai_events`, `invoices`, `partner_referrals`. The `ai_events` table is the single append-only store for all AI activity (agent changes, batches, undo/redo, conversations, messages), with a self-referencing `parent_id` for event chains and a `jsonb` payload. The `users` table consolidates former `team_members`, `profiles`, and `people` tables. The `tasks` table is the unified task store (replaces former `lead_tasks`). Task status values are `todo`, `in_progress`, `done`. Assignment uses `team_member_id` (FK to `users`).
